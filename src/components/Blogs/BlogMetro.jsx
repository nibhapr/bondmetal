            ...
            <li key={i} className={`grid-item${item.double_col ? " grid-item-double" : ""} ${item.category.map(item => item.split(" ").join("")).toString().split(",").join(" ").toLowerCase()}`} >
              <m.div className="blog-metro"
                ...
                <div className="post-details">
                  <a aria-label="link" to={`/blogs/category/${[item.category[0].toString().split(" ").join("").toLowerCase()]}`} className="post-btn"> {item.category[0]}</a>
                  <span className="post-date">
                    {item.date}
                  </span>
                  <a aria-label="link" to={`${props.link}${[item.id]}`} className="post-title">
                    {item.title}
                  </a>
                </div>
              </m.div>
